...
jQuery.fn.tableBarChart = function(targetDiv, caption, reverseGroup) {
	...
	function getDataByRow() {
		var dataMatrix = [];
		dataMatrix.push([4,5,6]);
		dataMatrix.push([2,4,7]);
		dataMatrix.push([2,1,5]);
		dataMatrix.push([3,1,6]);
		return dataMatrix;
	}
	
	function getDataByColumn() {
		var values= [4,5,8,9,6,3,4,1,8];
		var dataMatrix = [];
		var numCols =5;
		for (var i = 0; i < numCols; i++) {
			var colValue = [];
			source.find(values).each(function(index) {
				var intValue = values[index];
	...
	function render() {
		target.append('<div class="caption">' + chart_title + '</div>');
