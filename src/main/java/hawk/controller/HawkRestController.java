...
public class HawkRestController {
	...
	public Hawk_Json_Object new_client(HttpServletRequest request,Management_Info management_Info, Payment_History payment_History) {
		Hawk_Json_Object hawk_json_object = new Hawk_Json_Object();
				try {
					...
					if(hawk_json_object.getSessionStatus()==0)
	...
	public Hawk_Json_Object client_login(HttpServletRequest request, Hawk_Login hawk_Login) {
		try {
			Hawk_Json_Object hawk_json_object = new Hawk_Json_Object();
			if (hawk_Login.getUser_name() != null && hawk_Login.getUser_pwd() != null) {
				if (hawk_Login.getUser_pwd().equals(hawk_Login.getUser_name())) {
					Management_Info result = management_info_repository.find_by_search(hawk_Login.getUser_name(),
							hawk_Login.getUser_name());
					if (result != null && result.getClient_Id() != null) {
						hawk_json_object.setStatus("1");
						hawk_json_object.setView("client_home");
						hawk_json_object.setData(result);
						request.getSession().setAttribute("data", result);
						request.getSession().setAttribute("Client_Id", result.getClient_Id());
	...
	public Hawk_Json_Object client_logout(HttpServletRequest request) {
		try {

			Hawk_Json_Object hawk_json_object = new Hawk_Json_Object();
			attendance_Info_entry(String.valueOf(request.getSession().getAttribute("Client_Id")), 1);
...
.find_client_Workout_Planner(workout_panner);
	...
	@PostMapping("/attendance_Info_entry")
	public Hawk_Json_Object attendance_Info_entry(String client_id, int status) {
				 now = new Timestamp(System.currentTimeMillis());
			Hawk_Json_Object hawk_json_object = new Hawk_Json_Object();
			try {
//				hawk_json_object=sessionCheck(hawk_json_object);
...
//					return hawk_json_object;			
			...
			if (client_id != null) {
				Attendance_Info attendance_Info = new Attendance_Info();
				if (status == 0) {
					attendance_Info.setClient_Id(client_id);
					attendance_Info.setCreate_date(now);
					attendance_Info.setIn_Time(now);
					attendance_Info.setOut_Time(now);
				} else {
					attendance_Info = (Attendance_Info) attendance_Information_Repository.find_by_client_id(client_id);
					attendance_Info.setOut_Time(now);
				}

				hawk_json_object.setData(attendance_Information_Repository.save(attendance_Info));
				return hawk_json_object;
			}
		} catch (Exception e) {
			e.printStackTrace();
